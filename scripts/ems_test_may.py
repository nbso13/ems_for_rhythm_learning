...
#     audio_thread = threading.Thread(target=run_rhythm_audio, args= (ems_constants_may.audio_on_flags, phase_flags_list, audio_onset_times, time_naught_thread, phase_repeats_list, rhythm_substr, \
...
#             command_bytes = f"xC{str(ems_constants_may.channel)}I100T{str(ems_constants_may.actual_stim_length)}G \n " # metronome intro
...
def run_rhythm_ems_new(ems_serial, time_naught_thread,  \
    ...
    assert refractory_period_ms >= 1.5*ems_constants_may.actual_stim_length, "refrac_period_too_short!" # otherwise the refractory period does nothing.
    
    command_bytes = f'xC{ems_constants_may.channel}I100T{ems_constants_may.actual_stim_length}G' + ' \n' # metronome intro
    byt_com = bytes(command_bytes, encoding='utf8')
...
def run_rhythm_audio_new(milliseconds_per_eighthnote, time_naught_thread,  \
        audio_onset_times, audio_times, audio_times_dont_play, update_period_ms, phase_change_times, phase_warn_strs):
        # runs ems. vars:
    ...
    while len(audio_times) > 0 and not(next_stim == 0): 
        time.sleep(update_period_ms/1000)
        note_tone.stop()
        if ((time.time() - time_naught_thread) * 1000) > next_phase_change and phase_counter < len(phase_warn_strs):
            print(f" NEXT BLOCK NOW: {ems_constants_may.phase_warning_strs[phase_counter]}")
            next_phase_change = phase_change_times.pop(0)
            phase_counter += 1
        if ((time.time() - time_naught_thread)*1000) > next_stim:
...
#                 command_bytes = f'xC{str(ems_constants_may.channel)}I100T{str(actual_stim_length)}G \n' # metronome intro
#                 byt_com = bytes(command_bytes, encoding='utf8')
#                 stim_onset_times.append(time.time() - time_naught)
#                 ems_serial.write(byt_com)
#                 print("stim on")
...
#     AUDIO_DELAY = ems_constants_may.audio_delay
...
def read_contact_trace(ser, len_rhythm_presentation_ms, samp_period_ms, readings_list, x_values_list, time_naught_contact_trace):
    ...
    if ems_constants_may.sound_feedback_mode_flag == 1: # if we want feedback we have this loop
        while (time.time()-time_naught_contact_trace)*1000 < len_rhythm_presentation_ms + ems_constants_may.read_buffer_time_val:
            if ser.in_waiting:
                out = ser.readline().decode('utf-8')
                time_measured = time.time()
                if int(out[:-2]) > ems_constants_may.baseline_subtractor:
                    # print(int(out[:-2]))
            ...
            if (unplayed == False and time_measured > 0.2 + play_time) and (np.mean(readings_list[-10:-1]) < ems_constants_may.baseline_subtractor): 
...
def read_keyboard_trace(len_rhythm_presentation_ms, samp_period_ms, readings_list, x_values_list, time_naught_contact_trace):
    ...
    while (time.time()-time_naught_contact_trace)*1000 < len_rhythm_presentation_ms:
        time.sleep(ems_constants_may.update_period_ms/1000)
...
def measure_delay(actual_stim_length, trial_num, sleep_len, samp_period_ms, sd_more_than_mult, baseline_subtractor, baseline_mean, baseline_sd, ems_serial=0, contact_ser=0):
    ...
    if ems_constants_may.delay_mode == 'key':
    ...
    elif ems_constants_may.delay_mode == 'contact':
        read_thread = threading.Thread(target=read_contact_trace, args= (contact_ser, len_pres,  \
            samp_period_ms, reading_results, x_value_results, time_naught_delay))
...
def measure_delay_loop(baseline_mean=100, baseline_sd = 0, ems_serial=0, contact_serial=0):  
    ...
    if out == 'y':
        ...
        while(repeat_bool):
            if ems_constants_may.input_mode == 'contact':
                ...
                reading_results, contact_x_values = measure_delay(ems_constants_may.actual_stim_length, \
                    ems_constants_may.delay_trial_num, ems_constants_may.sleep_len_ms, ems_constants_may.samp_period_ms,  \
                    ems_constants_may.sd_more_than_mult, ems_constants_may.baseline_subtractor, baseline_mean, baseline_sd, ems_serial = ems_serial, contact_serial = contact_serial)
            elif ems_constants_may.input_mode == 'key':
                delay_mean, delay_std, reaction_onsets, stim_onsets, \
                reading_results, contact_x_values = measure_delay(ems_constants_may.actual_stim_length, \
                    ems_constants_may.delay_trial_num, ems_constants_may.sleep_len_ms, ems_constants_may.samp_period_ms,  \
                    ems_constants_may.sd_more_than_mult, ems_constants_may.baseline_subtractor, baseline_mean, baseline_sd)
            
            x_vec = np.arange(0, np.max(contact_x_values), ems_constants_may.samp_period_ms)
            stim_trace = spike_times_to_traces(stim_onsets, ems_constants_may.actual_stim_length, x_vec, ems_constants_may.samp_period_ms)
            reaction_trace = spike_times_to_traces(reaction_onsets, ems_constants_may.actual_stim_length, x_vec, ems_constants_may.samp_period_ms)

            # x_vec, reaction_trace, stim_trace = onset_times_to_traces(reaction_onsets, ems_constants_may.contact_spike_time_width, stim_onsets, ems_constants_may.actual_stim_length, ems_constants_may.samp_period_ms)
            ...
            ems_constants_may.samp_period_ms, legend_labels, "Delay Measurement")
...
def author_time_course(rhythm_substr, milliseconds_per_eighthnote, phase_repeats_list, phase_flags_list):
    ...
    for phase_repeats_index in range(len(phase_repeats_list)):
        if phase_flags_list[phase_repeats_index]:
            if ems_constants_may.ems_on_flags[phase_repeats_index]:
                for j in range(phase_repeats_list[phase_repeats_index]):
    ...
    for phase_repeats_index in range(len(phase_repeats_list)):
        if phase_flags_list[phase_repeats_index]:
            ...
            if ems_constants_may.audio_on_flags[phase_repeats_index]:
                for j in range(phase_repeats_list[phase_repeats_index]):
...
def play_rhythm_new(actual_stim_length, rhythm_substr, bpm,  phase_flags_list, phase_repeats_list,  \
    samp_period_ms, delay_val, prep_time_ms, update_period_ms, refractory_period_ms, phase_warn_strs, ems_serial=0, contact_ser=0):
    # mammoth function that should be broken up. takes in serial objects, rhythm parameters,
        # whether the EMS and the audio should be turned on, whether there should be a metronome intro,
        # sample period, and measured delay value. Plays the rhythm in audio and EMS and runs threading
        # to listen to user results.
...
# total eighthnotes in count in, audio display, and rhythm display (EMS+audio)
    ...
    if ems_constants_may.input_mode == 'contact':
        ...
        read_thread = threading.Thread(target=read_keyboard_trace, args= (len_pres,  \
            samp_period_ms, reading_results, x_value_results, time_naught_thread))
        read_thread.start() 
        audio_thread = threading.Thread(target=run_rhythm_audio_new, args= (milliseconds_per_eighthnote, time_naught_thread,  \
            audio_onset_times, audio_times_adjusted, audio_times_dont_play, update_period_ms, phase_change_times_final, phase_warn_strs))
        metronome_thread = threading.Thread(target=metronome_tone_new, args= (milliseconds_per_eighthnote, time_naught_thread, \
        ...
        audio_onset_times_ms = [1000 * item for item in audio_onset_times] # take list of onset times from seconds to ms.
        # reading results is the contact trace list. x value results are time of recording each data point.
        return reading_results, x_value_results, audio_onset_times_ms 
...
def initial_preprocess_and_plot(reading_list, contact_x_values, bpm, rhythm, rhythm_name, stim_onset_times, audio_onset_times, start_time_stim):
    ...
    x_vec = np.arange(0, np.max(contact_x_values), ems_constants_may.samp_period_ms)
    audio_trace = spike_times_to_traces(audio_onset_times, audio_hold, x_vec, ems_constants_may.samp_period_ms)
    if ems_constants_may.ems_on == 1:
        stim_trace = spike_times_to_traces(stim_onset_times, ems_constants_may.actual_stim_length,  \
            x_vec, ems_constants_may.samp_period_ms)
    else:
        stim_trace = np.zeros_like(x_vec)
    ...
    plot_contact_trace_and_rhythm_and_last_milliseconds(reading_list, contact_x_values, stim_trace, audio_trace,  \
        x_vec, ems_constants_may.samp_period_ms, legend_labels, input_title, last_milliseconds_amount, start_time_stim)
...
def write_header_and_data_vals(data_header, arrs_to_write, worksheet, bold):
    ...
    for i in range(len(arrs_to_write)):
        for row_num, data in enumerate(arrs_to_write[i]):
            worksheet.write(row_num + ems_constants_may.worksheet_data_begin_indices[0], ems_constants_may.worksheet_data_begin_indices[1] + i, data)
    return

def practice_loop(measured_delay, rhythm, bpm, rhythm_name, ems_serial=0, contact_serial=0):
    out = input("Practice initiate? y/n")
    phase_flag_list = ems_constants_may.phase_flags_list[0:round(len(ems_constants_may.phase_flags_list)/8)]
    phase_repeat_list = ems_constants_may.phase_repeats_list[0:round(len(ems_constants_may.phase_repeats_list)/8)]
    phase_warn_strs = ems_constants_may.phase_warning_strs[0:round(len(ems_constants_may.phase_warning_strs)/8)]
    if out == 'y':
        repeat = True
        while repeat:
            if ems_constants_may.input_mode == 'contact' and ems_constants_may.ems_on == 1:
                reading_list, contact_x_values, audio_onset_times, stim_onset_times = play_rhythm_new(ems_constants_may.actual_stim_length,  rhythm, \
                    bpm, phase_flag_list, phase_repeat_list, ems_constants_may.samp_period_ms, measured_delay,\
                        ems_constants_may.prep_time_ms, ems_constants_may.update_period_ms, ems_constants_may.refractory_period_ms, phase_warn_strs, ems_serial=ems_serial, contact_serial=contact_serial)
            elif ems_constants_may.input_mode == 'contact' and ems_constants_may.ems_on == 0:
                reading_list, contact_x_values, audio_onset_times, stim_onset_times = play_rhythm_new(ems_constants_may.actual_stim_length,  rhythm, \
                    bpm, phase_flag_list, phase_repeat_list, ems_constants_may.samp_period_ms, measured_delay,\
                        ems_constants_may.prep_time_ms, ems_constants_may.update_period_ms, ems_constants_may.refractory_period_ms, phase_warn_strs, contact_serial=contact_serial)
                ...
                reading_list, contact_x_values, audio_onset_times  = play_rhythm_new(ems_constants_may.actual_stim_length,  rhythm, \
                     bpm, phase_flag_list, phase_repeat_list, ems_constants_may.samp_period_ms, measured_delay,\
                        ems_constants_may.prep_time_ms, ems_constants_may.update_period_ms, ems_constants_may.refractory_period_ms, phase_warn_strs)
                stim_onset_times = [audio_onset_times[0]]
...
def tester():
    # reading_list, contact_x_values, audio_onset_times_new, stim_onset_times = play_rhythm_new(ems_serial, contact_serial, ems_constants_may.actual_stim_length,  rhythm_substr, \
    #             bpm, ems_constants_may.phase_flags_list, ems_constants_may.phase_repeats_list, ems_constants_may.samp_period_ms, delay_mean,\
    #                 ems_constants_may.prep_time_ms, ems_constants_may.update_period_ms, ems_constants_may.refractory_period_ms)
...
if __name__ == '__main__':
    ...
    if ems_constants_may.input_mode == 'contact':
        ems_serial, contact_serial, = set_up_serials(ems_constants_may.port_ems, ems_constants_may.port_contact)
    ...
    ### run main experiment ###
    for rhythm_index in range(len(ems_constants_may.rhythm_strings)): # for each of the different rhythms
        ...
        ### PLAY RHYTHM ###
        if ems_constants_may.input_mode == 'contact' and ems_constants_may.ems_on == 1:
            reading_list, contact_x_values, audio_onset_times, stim_onset_times = play_rhythm_new(ems_constants_may.actual_stim_length,  rhythm_substr, \
                bpm, ems_constants_may.phase_flags_list, ems_constants_may.phase_repeats_list, ems_constants_may.samp_period_ms, delay_mean,\
                ems_constants_may.prep_time_ms, ems_constants_may.update_period_ms, ems_constants_may.refractory_period_ms, ems_constants_may.phase_warning_strs, ems_serial=ems_serial, contact_serial=contact_serial)
        elif ems_constants_may.input_mode == 'contact' and ems_constants_may.ems_on == 0:
            reading_list, contact_x_values, audio_onset_times, stim_onset_times = play_rhythm_new(ems_constants_may.actual_stim_length,  rhythm_substr, \
                bpm, ems_constants_may.phase_flags_list, ems_constants_may.phase_repeats_list, ems_constants_may.samp_period_ms, delay_mean,\
                ems_constants_may.prep_time_ms, ems_constants_may.update_period_ms, ems_constants_may.refractory_period_ms, ems_constants_may.phase_warning_strs, contact_serial=contact_serial)
            stim_onset_times = [audio_onset_times[0]]
        elif ems_constants_may.input_mode == 'key':
            reading_list, contact_x_values, audio_onset_times = play_rhythm_new(ems_constants_may.actual_stim_length,  rhythm_substr, \
                bpm, ems_constants_may.phase_flags_list, ems_constants_may.phase_repeats_list, ems_constants_may.samp_period_ms, delay_mean,\
    ...
    write_study_info_and_save(workbook, f"data/{participant_info_dictionary['test time']}_pp{participant_info_dictionary['pp number']}_header_info.pkl",  \
        f"data/{participant_info_dictionary['test time']}_pp{participant_info_dictionary['pp number']}_header_info.txt", \
             participant_info_dictionary, ems_constants_may.runtime_parameters)
...
#     ## further processing
#     surpressed_contact_onset_times = process_contact_trace_to_hit_times(reading_list, contact_x_values, ems_constants_may.baseline_subtractor, ems_constants_may.surpression_window)

#     contact_hold = ems_constants_may.contact_spike_time_width
#     surpressed_contact_trace = spike_times_to_traces(surpressed_contact_onset_times, contact_hold, x_vec, ems_constants_may.samp_period_ms)

#     legend_labels = ["surpressed contact trace", "stim trace", "audio trace"]

#     plot_contact_trace_and_rhythm(surpressed_contact_trace, x_vec, stim_trace, audio_trace, x_vec, ems_constants_may.samp_period_ms, legend_labels)
...
#     len_rhythm_ms = len(rhythm_substr) * ems_constants_may.milliseconds_per_eighthnote
#     len_count_off_ms = len(ems_constants_may.count_in_substr) * ems_constants_may.milliseconds_per_eighthnote 
#     len_count_off_and_audio_display_ms = len_count_off_ms + ems_constants_may.audio_repeats*len_rhythm_ms
#     len_count_off_and_audio_display_and_ems_ms = len_count_off_ms + ems_constants_may.audio_repeats*len_rhythm_ms + ems_constants_may.repeats*len_rhythm_ms
...
#     distances_list = []
#     repeat_list = [ems_constants_may.audio_repeats, ems_constants_may.repeats, ems_constants_may.post_ems_repeats]
...
#             loop_begin = delays_list[i] + j * len_rhythm_ms - ems_constants_may.milliseconds_per_eighthnote #include one eighthnote before
#             loop_end = loop_begin + len_rhythm_ms + 2 * ems_constants_may.milliseconds_per_eighthnote #include one eighthnote after as well
